...
exports.AviaryWidget = exports.FilepickerPictureWidget.extend({
    init: function (options) {
        ...
        if ('sizeConstraint' in options) this.data['sizeConstraint'] = options.sizeConstraint;
    }
});
...
exports.AutocompleteWidget = exports.TextWidget.extend({
    init: function (options) {
        if (!options.url && !options.socket) throw new Error('must specify url');
        if (!options.ref) throw new TypeError('model was not provided');

        this._super(options);

        if(options.updates) {
            this.attrs.class.push('nf_ref_update');
        } else {
